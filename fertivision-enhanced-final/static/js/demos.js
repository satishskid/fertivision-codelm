...
class FertiVisionDemos {
    ...
    initializeEventListeners() {
        document.addEventListener('click', (e) => {
            ...
            } else if (e.target.classList.contains('dataset-btn')) {
                const datasetName = e.target.getAttribute('data-dataset');
                this.loadHuggingFaceDataset(datasetName);
            } else if (e.target.classList.contains('sample-btn')) {
                const sampleType = e.target.getAttribute('data-sample');
                this.loadSampleFromDataset(sampleType);
            }
        });
    }
    ...
    async loadHuggingFaceDataset(datasetName) {
        ...
        resultsContainer.innerHTML = `
            <div class="dataset-loading">
                <div class="loading-spinner"></div>
                <h3>🤗 Loading Hugging Face Dataset</h3>
                <p>Fetching ${datasetName} dataset samples...</p>
            </div>
        `;

        try {
            // Simulate dataset loading (in real implementation, this would call HF API)
            await new Promise(resolve => setTimeout(resolve, 2000));

            const datasetInfo = this.getDatasetInfo(datasetName);
            this.displayDatasetSamples(resultsContainer, datasetInfo);
            ...
            resultsContainer.innerHTML = `
                <div class="dataset-error">
                    ...
                    <p>Could not load ${datasetName} dataset. Please try again.</p>
                    <button class="btn btn-primary" onclick="window.FertiVisionDemos.clearDemoResults()">
                        Back to Demos
                    </button>
                </div>
            `;
    ...
    getDatasetInfo(datasetName) {
        const datasets = {
            'medical-imaging': {
                ...
                description: 'Curated medical images for reproductive medicine analysis',
                samples: [
                    { type: 'sperm', name: 'Sperm Microscopy Sample 1', url: 'sample-sperm-1.jpg' },
                    { type: 'sperm', name: 'Sperm Microscopy Sample 2', url: 'sample-sperm-2.jpg' },
                    { type: 'oocyte', name: 'Oocyte MII Sample', url: 'sample-oocyte-1.jpg' },
                    { type: 'embryo', name: 'Day 3 Embryo Sample', url: 'sample-embryo-1.jpg' },
                    { type: 'follicle', name: 'Ovarian Ultrasound Sample', url: 'sample-follicle-1.jpg' }
                ],
                totalSamples: 150,
                categories: ['sperm', 'oocyte', 'embryo', 'follicle', 'hysteroscopy']
            },
            'fertility-research': {
                name: 'Fertility Research Dataset',
                description: 'Research-grade images for AI model training',
                samples: [
                    { type: 'sperm', name: 'WHO Reference Sample', url: 'who-sperm-ref.jpg' },
                    { type: 'embryo', name: 'Gardner Grade A Embryo', url: 'gardner-a-embryo.jpg' },
                    { type: 'oocyte', name: 'ESHRE Standard Oocyte', url: 'eshre-oocyte.jpg' }
                ],
                totalSamples: 500,
                categories: ['sperm', 'oocyte', 'embryo']
            }
        };

        return datasets[datasetName] || datasets['medical-imaging'];
    }

    /**
     * Display dataset samples
     */
    displayDatasetSamples(container, datasetInfo) {
        container.innerHTML = `
            <div class="dataset-display">
                <div class="dataset-header">
                    ...
                    <div class="dataset-stats">
                        <span class="stat">📊 ${datasetInfo.totalSamples} samples</span>
                        <span class="stat">🏷️ ${datasetInfo.categories.length} categories</span>
                    </div>
                </div>
                            ...
                            <div class="sample-card">
                                <div class="sample-image">
                                    <div class="sample-placeholder">
                                        ${this.getSampleIcon(sample.type)}
                                    </div>
                                </div>
                                ...
                                <div class="sample-actions">
                                    <button class="btn btn-primary sample-btn" data-sample="${sample.type}">
                                        Analyze with AI
                                    </button>
                                </div>
                            </div>
                ...
                <div class="dataset-actions">
                    ...
                    <button class="btn btn-primary" onclick="window.FertiVisionDemos.loadRandomSample()">
        ...
        // Add dataset-specific styling
        this.addDatasetStyling();
    }

    /**
     * Get sample icon for type
     */
    getSampleIcon(type) {
        const icons = {
            sperm: '🧬',
    ...
    /**
     * Load sample from dataset for analysis
     */
    async loadSampleFromDataset(sampleType) {
        console.log(`🔬 Loading ${sampleType} sample for analysis...`);

        // Run the corresponding demo analysis
        await this.runDemo(sampleType);
    }
     ...
     */
    loadRandomSample() {
        const types = ['sperm', 'oocyte', 'embryo', 'follicle', 'hysteroscopy'];
        const randomType = types[Math.floor(Math.random() * types.length)];
        this.loadSampleFromDataset(randomType);
    }
     ...
     */
    addDatasetStyling() {
        if (document.getElementById('dataset-styles')) return;

        const style = document.createElement('style');
        style.id = 'dataset-styles';
        style.textContent = `
            .dataset-loading {
                text-align: center;
                padding: 3rem;
