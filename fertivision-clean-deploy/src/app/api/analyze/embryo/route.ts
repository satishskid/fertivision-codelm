...
export async function POST(request: NextRequest) {
  try {
    ...
    const day = formData.get('day') as string; // Development day for embryo

    if (!image || !image.type.startsWith('image/')) {
      return NextResponse.json({ success: false, error: 'Valid image file required' }, { status: 400 });
    }

    // Add day information to notes if provided
    const enhancedNotes = day ? `${additionalNotes ? additionalNotes + '. ' : ''}Development day: ${day}` : additionalNotes;

    const analysisResult = await analyzeImage(image, 'embryo', patientId, caseId, enhancedNotes);
    ...
    const savedAnalysis = await saveAnalysis(
      user.id, 'embryo', {
        ...
        parameters: { ...analysisResult.parameters, day: day || 'not_specified' },
    ...
    console.error('Embryo analysis error:', error);
